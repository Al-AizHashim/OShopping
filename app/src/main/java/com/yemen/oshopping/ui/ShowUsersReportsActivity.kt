...
class ShowUsersReportsActivity : AppCompatActivity() {
        ...
        tabViewPager.adapter = object : FragmentStateAdapter(this) {
            override fun createFragment(position: Int): Fragment {
                return when (position) {
                    0 -> ShowReportsFragment.newInstance(position)
                    1 -> ShowReportsFragment.newInstance(position)
                    2 -> ShowReportsFragment.newInstance(position)
                    else -> ShowReportsFragment.newInstance(0)
                }
            }
